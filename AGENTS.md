...
## Testing Gotchas
- Running tests: `tests/track_prediction_test.rs` (3 tests), `tests/impact_prediction_test.rs` (2 tests), `tests/interceptor_engagement_test.rs` (4 tests), `tests/scenario_builder_test.rs` (5 tests: TOML round-trip on all scenarios, Option-field handling, draft→engine chain, import flow), `tests/alert_assessment_test.rs` (3 tests: DEFCON escalation from real fused tracks, klaxon gate, quiet engine), `tests/measurement_error_test.rs` (2 tests: azimuth bias displaces fused tracks, zero-bias noise stays bounded), `tests/discrimination_test.rs` (3 tests: decoys spawn/tracked, classification separates decoy from RV, likely-decoy tracks not engaged) — plus `#[cfg(test)]` unit tests in `src/scenario/builder.rs` (7 tests, run in both lib and bin targets), `src/simulation/physics.rs` (10: WGS-84 geodesy + Coriolis), `src/simulation/alert.rs` (11), `src/simulation/detection.rs` (5 clutter tests), `src/tracking/mod.rs` (5), `src/audio/mod.rs` (3).
